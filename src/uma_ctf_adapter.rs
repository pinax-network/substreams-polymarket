    ...
    for trx in block.transactions() {
        ...
        for log_view in trx.receipt().logs() {
            ...
            if let Some(event) = events::QuestionFlagged::match_and_decode(log) {
                ...
                let event =
                    pb::log::Log::UmaCtfAdapterQuestionFlagged(pb::UmaCtfAdapterQuestionFlagged {
                        question_id: event.question_id.to_vec(),
                    });
                transaction.logs.push(pb::Log::create_log(log, event));
                continue;
            ...
            if let Some(event) = events::QuestionInitialized::match_and_decode(log) {
                ...
                let event = pb::log::Log::UmaCtfAdapterQuestionInitialized(
                    pb::UmaCtfAdapterQuestionInitialized {
                        question_id: event.question_id.to_vec(),
            ...
            if let Some(event) = events::QuestionPaused::match_and_decode(log) {
                ...
                let event =
                    pb::log::Log::UmaCtfAdapterQuestionPaused(pb::UmaCtfAdapterQuestionPaused {
                        question_id: event.question_id.to_vec(),
                    });
                transaction.logs.push(pb::Log::create_log(log, event));
                continue;
            ...
            if let Some(event) = events::QuestionReset::match_and_decode(log) {
                ...
                let event =
                    pb::log::Log::UmaCtfAdapterQuestionReset(pb::UmaCtfAdapterQuestionReset {
                        question_id: event.question_id.to_vec(),
                    });
                transaction.logs.push(pb::Log::create_log(log, event));
                continue;
            ...
            if let Some(event) = events::QuestionResolved::match_and_decode(log) {
                ...
                let event = pb::log::Log::UmaCtfAdapterQuestionResolved(
                    pb::UmaCtfAdapterQuestionResolved {
                        question_id: event.question_id.to_vec(),
            ...
            if let Some(event) = events::QuestionUnpaused::match_and_decode(log) {
                ...
                let event = pb::log::Log::UmaCtfAdapterQuestionUnpaused(
                    pb::UmaCtfAdapterQuestionUnpaused {
                        question_id: event.question_id.to_vec(),
                    },
                );
                transaction.logs.push(pb::Log::create_log(log, event));
            ...
            if let Some(event) = events::RemovedAdmin::match_and_decode(log) {
                ...
                let event =
                    pb::log::Log::UmaCtfAdapterRemovedAdmin(pb::UmaCtfAdapterRemovedAdmin {
                        admin: event.admin.to_vec(),
            ...
            if let Some(event) = events::QuestionUnflagged::match_and_decode(log) {
                ...
                let event = pb::log::Log::UmaCtfAdapterQuestionUnflagged(
                    pb::UmaCtfAdapterQuestionUnflagged {
                        question_id: event.question_id.to_vec(),
                    },
                );
                transaction.logs.push(pb::Log::create_log(log, event));
