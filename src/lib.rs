...
pub fn map_events(
    ...
    events.transactions.extend(
        ctf_exchange::map_events(CTF_EXCHANGE_FILTER.to_string(), block.clone())?.transactions,
    );
    events.transactions.extend(
        uma_ctf_adapter::map_events(UMA_CTF_ADAPTER_FILTER.to_string(), block.clone())?
            .transactions,
    );
    events.transactions.extend(
        negrisk_adapter::map_events(NEGRISK_ADAPTER_FILTER.to_string(), block.clone())?
            .transactions,
    );
    events.transactions.extend(
        conditional_tokens::map_events(CONDITIONAL_TOKENS_FILTER.to_string(), block.clone())?
            .transactions,
    );
    events.transactions.extend(
        safe_proxy_factory::map_events(SAFE_PROXY_FACTORY_FILTER.to_string(), block.clone())?
            .transactions,
    );
    events
        .transactions
        .extend(fee_module::map_events(FEE_MODULE_FILTER.to_string(), block)?.transactions);
