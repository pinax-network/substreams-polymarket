...
-- from CTFExchange OrderFilled events.
--
-- Two MVs write to this table:
--   * mv_refresh_state_user_position         — hourly, intervals 60/1440/10080/43200
--   * mv_refresh_state_user_position_alltime — every 6h, interval_min = 0
--
-- Consumers must read with FINAL.
...
-- Maker/taker legs UNION ALL; buy/sell direction from which leg carries USDC (asset_id = 0).
...
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_refresh_state_user_position_alltime
REFRESH EVERY 6 HOUR OFFSET 42 MINUTE APPEND
TO state_user_position
AS
WITH
    sides AS (
